...
def clean_str(string):
    ...
    该代码中的评论信息user_review　and item_review只包含训练集的评论数据我认为这有点问题
    """
...
def build_vocab(sentences1,sentences2):
    ...
    # Mapping from index to word
    vocabulary_inv2 = [x[0] for x in word_counts2.most_common()] #安词出现的次数排序

    vocabulary_inv2 = list(sorted(vocabulary_inv2))#按词典字幕大小排序　　
...
def load_data(train_data,valid_data,user_review,item_review):
    ...
    # 此处的u_text已经成为字典　评论也单词数字化了
    # print "dddddddddddddd" ,u_text[2].shape[0]
...
def load_data_and_labels(train_data,valid_data,user_review,item_review):
    ...
    for line in f_train:
        ...
        if u_text.has_key(int(line[0])):
            ...
            for s in user_reviews[int(line[0])]:
                u_text[int(line[0])] = u_text[int(line[0])] + ' ' + s.strip()#连接评论
            u_text[int(line[0])]=clean_str(u_text[int(line[0])])#清除字符
            u_text[int(line[0])]=u_text[int(line[0])].split(" ")#分开为单词
    ...
    for line in f_valid:
        ...
        if u_text.has_key(int(line[0])):
            ...
            u_text[int(line[0])] = '<PAD/>'#训练集合中可能没有这个user_id,那么这个user_id 的U_text就添加'<PAD/>'
    ...
    print ">>>>>>>>>>>>>>>>",u,len(u)
    x = np.sort(u)#把每一个用户的评论单词的个数排序
    print ">>>>>>>>>>>>>>>>", x
    u_len = x[int(0.85* len(u)) - 1]#然后按照比例随机选了一个个数当做评论的长度
...
if __name__ == '__main__':
    ...
    para['user_length']=u_text[0].shape[0] #每一个user的维度，也就是选了几个单词
